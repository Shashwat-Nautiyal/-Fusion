...
contract EscrowFactory {
    event CreatedEscrow(address indexed escrow, address indexed creator, bool isSrc);
   ...
   function createSrcEscrow(
    ...
    uint256 amount,
    bytes32 salt
) external payable returns (address) {  // Add return type
    ...
    emit CreatedEscrow(address(escrow), msg.sender,true);
    return address(escrow);  // Add return statement
}
 ...
 function createDstEscrow(
    ...
    emit CreatedEscrow(address(escrow), msg.sender,false);
    return address(escrow);  // Add return statement
}
